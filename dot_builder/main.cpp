...
void generate_static_dot(const std::string& static_bin_path, const std::string& out_dot_path) {
    ...
    for (auto &pb_node : static_graph.nodes()) {
        ...
        switch (node_type) {
            case gb::Instr: builder.create_node<gb::Instr>(node_id); continue;
            case gb::Value: builder.create_node<gb::Value>(node_id); continue;
    ...
    for (auto &pb_edge : static_graph.edges()) {
        ...
        switch (edge_type) {
            case gb::Flow: builder.create_edge<gb::Flow>(edge_id); continue;
            case gb::Data: builder.create_edge<gb::Data>(edge_id); continue;
            case gb::Call: builder.create_edge<gb::Call>(edge_id); continue;
    ...
    for (auto &pb_cluster : static_graph.clusters()) {
        ...
        switch (cluster_type) {
            case gb::BB: builder.create_cluster<gb::BB>(cluster_id); continue;
            case gb::F: builder.create_cluster<gb::F>(cluster_id); continue;
